...
namespace Preprocessor
{
    ...
    public class MeshEncoder
    {
        ...
        void ReassignNodeNumber()
        {
            ...
            {
                for (int i = 0; i < triangle.Length; ++i) triangle[i] = table[triangle[i]];
                mesh.NormalizeNodeOrderOfPolygon(triangle);
            }
            foreach (var quadrangle in mesh.quadrangles)
            {
                for (int i = 0; i < quadrangle.Length; ++i) quadrangle[i] = table[quadrangle[i]];
                mesh.NormalizeNodeOrderOfPolygon(quadrangle);
            }
